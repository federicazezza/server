...
/**
  ...
  @param having_pushdown  TRUE <=> used for condition pushdown from
                          HAVING into WHERE
                                           ...
                                           bool having_pushdown)
{
  ...
  /*
    Create multiple equalities from new_conds list equalities.
  ...
  */
      ...
      ((Item_cond*) cond)->functype() == Item_func::COND_AND_FUNC)
  {
    /**
      ...
      new_cond_equal and the remain conditions from new_conds.
    */
    ...
    /**
      ...
      Merge these multiple equalities with new_cond_equal multiple
      equalities. Save result in new_cond_equal.
      Check if after the merge some equalities are knowingly true or false.
    */
    ...
    {
      /**
        Case when and_new_conditions_to_optimized_cond() is called for
        HAVING pushdown optimization.

        In pushdown_from_having_into_where() all multiple equalities
        that can pushed are transformed into equalities.
        When they are joined to optimized condition they should be
        transformed back to multiple equalities.

        Top level equalities are transformed into multiple equalities
        above.
        Bottom level equalities are transformed here (equalities that are
        not on the top level).
        Examples of bottom level equalities:

        1. (t1.a = 3) OR (t1.b > 5)
            (t1.a = 3) - bottom level equality.
            It is inside OR condition

        2. ((t3.d = t3.c) AND (t3.c < 15)) OR (t3.d > 1)
           (t1.d = t3.c) - bottom level equality.
           It is inside AND condition which is a part of OR condition
        ...

        To create multiple equalities build_equal_items() is called.
      */
      if (having_pushdown &&
  ...
  else
  {
    ...
    */
    ...
    /**
      Fix created equalities and check if they are knowingly true or false.
    */
